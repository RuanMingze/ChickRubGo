...
const DB_VERSION = 3; // 增加版本号以支持壁纸存储
const AUDIO_STORE_NAME = 'audioStore';
const SETTINGS_STORE_NAME = 'settingsStore';
const WALLPAPER_STORE_NAME = 'wallpaperStore'; // 新增壁纸存储
let db = null;
	...
	return new Promise((resolve, reject) => {
		...
		request.onupgradeneeded = (event) => {
			...
			if (!database.objectStoreNames.contains(AUDIO_STORE_NAME)) {
				const objectStore = database.createObjectStore(AUDIO_STORE_NAME, {
					keyPath: 'id'
				});
			...
			if (!database.objectStoreNames.contains(SETTINGS_STORE_NAME)) {
				const settingsStore = database.createObjectStore(SETTINGS_STORE_NAME, {
					keyPath: 'userId'
				});
			...
			if (!database.objectStoreNames.contains(WALLPAPER_STORE_NAME)) {
				const wallpaperStore = database.createObjectStore(WALLPAPER_STORE_NAME, {
					keyPath: 'id'
				});
				wallpaperStore.createIndex('timestamp', 'timestamp', {
					unique: false
				});
			}
		};
...
// 保存壁纸到 IndexedDB
async function saveWallpaperToIndexedDB(wallpaperData, wallpaperName) {
	if (!db) {
		await initIndexedDB();
	}
	...
	return new Promise((resolve, reject) => {
		...
		const objectStore = transaction.objectStore(WALLPAPER_STORE_NAME);

		const wallpaperRecord = {
			id: 'wallpaper',
			name: wallpaperName || 'custom',
			data: wallpaperData,
			timestamp: Date.now()
		};

		const request = objectStore.put(wallpaperRecord);
		...
		request.onerror = () => {
			console.error('壁纸保存到 IndexedDB 失败:', request.error);
			reject(request.error);
		};
	});
}
...
// 从 IndexedDB 加载壁纸
async function loadWallpaperFromIndexedDB() {
	if (!db) {
		await initIndexedDB();
	}
	...
	return new Promise((resolve, reject) => {
		...
		const objectStore = transaction.objectStore(WALLPAPER_STORE_NAME);
		const request = objectStore.get('wallpaper');
		...
		request.onsuccess = () => {
			if (request.result) {
				...
				if (savedWallpaperBase64) {
					// 将本地存储的壁纸迁移到 IndexedDB
					saveWallpaperToIndexedDB(savedWallpaperBase64, 'migrated').catch(err => {
						console.error('迁移壁纸到 IndexedDB 失败:', err);
					});
					resolve({
						data: savedWallpaperBase64,
						name: 'migrated',
						timestamp: savedWallpaperTime ? parseInt(savedWallpaperTime) : Date.now()
					});
				} else {
					resolve(null);
				}
		...
		request.onerror = () => {
			console.error('从 IndexedDB 加载壁纸失败:', request.error);
			// 尝试从本地存储读取
			const savedWallpaperBase64 = localStorage.getItem('wallpaperBase64');
			const savedWallpaperTime = localStorage.getItem('wallpaperTime');
			if (savedWallpaperBase64) {
...
async function loadWallpaper(skipCache = false) {
	...
	// 从IndexedDB加载壁纸
	let savedWallpaperData = null;
	try {
		savedWallpaperData = await loadWallpaperFromIndexedDB();
	} catch (error) {
		console.error('从IndexedDB加载壁纸失败:', error);
	}

	const FIVE_DAYS = 5 * 24 * 60 * 60 * 1000;
	const currentTime = Date.now();
	const isExpired = savedWallpaperData && savedWallpaperData.timestamp && (currentTime - savedWallpaperData.timestamp) > FIVE_DAYS;

	if (!skipCache && savedWallpaperData && !isExpired) {
		document.body.style.backgroundImage = `url(${savedWallpaperData.data})`;
	...
	try {
		...
		// 存储到IndexedDB
		try {
			await saveWallpaperToIndexedDB(base64, 'random');
		} catch (error) {
			console.error('壁纸保存到IndexedDB失败:', error);
			// 降级到localStorage
...
function startVoiceRecognition() {
	...
	if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
		...
		recognition.continuous = true; // 持续识别，不会自动终止
		...
		voiceIcon.className = 'fa-solid fa-stop'; // 显示停止图标
		voiceIcon.style.color = 'var(--secondary)'; // 录音时图标变为粉色
		...
		recognition.onend = () => {
			...
			if (isRecognizing) {
				...
				voiceIcon.style.color = ''; // 恢复图标默认颜色
			}
		};
		...
		recognition.onerror = (event) => {
			...
			// 根据错误类型显示不同的提示信息
			// 注意：aborted错误不再显示提示，因为可能是用户手动停止
			if (event.error === 'not-allowed') {
				...
				voiceIcon.className = 'fa-solid fa-microphone';
				voiceIcon.style.color = ''; // 恢复图标默认颜色
				ShowAlert('错误', '语音识别权限被拒绝，请在浏览器设置中允许麦克风访问');
			} else if (event.error === 'no-speech') {
				// 没有检测到语音
				// 继续语音识别，不停止
				ShowAlert('提示', '未检测到语音，请尝试更清晰地说话', true, 2000);
			} else if (event.error !== 'aborted') {
				// 其他错误（排除aborted）
				// 继续语音识别，不停止
				ShowAlert('错误', '语音识别失败，请重试');
			}
		};
	} else {
		// 浏览器不支持语音识别
		ShowAlert('提示', '您的浏览器不支持语音识别功能');
	}
}
...
const customWallpaperInfo = document.getElementById('custom-wallpaper-info');
const wallpaperBase64Input = document.getElementById('wallpaper-base64');
...
function loadSettings() {
	...
	// 加载自定义壁纸设置
	// 从IndexedDB加载壁纸信息
	loadWallpaperFromIndexedDB().then(wallpaperData => {
		if (wallpaperData && customWallpaperInfo) {
			customWallpaperInfo.textContent = '已设置自定义壁纸';
		}
	}).catch(error => {
		console.error('从IndexedDB加载壁纸信息失败:', error);
		// 降级到localStorage
		const savedWallpaperBase64 = localStorage.getItem('wallpaperBase64');
		if (savedWallpaperBase64 && customWallpaperInfo) {
			customWallpaperInfo.textContent = '已设置自定义壁纸';
		}
	});
...
function initCustomWallpaper() {
	if (customWallpaperInput) {
		customWallpaperInput.addEventListener('change', handleCustomWallpaper);
	}

	if (applyBase64Btn) {
		applyBase64Btn.addEventListener('click', handleBase64Wallpaper);
	}
}

// 处理自定义壁纸选择
async function handleCustomWallpaper(event) {
	const file = event.target.files[0];
	if (!file) return;

	// 验证文件类型
	if (!file.type.startsWith('image/')) {
		ShowAlert('错误', '请选择有效的图片文件！', true, 3000);
		return;
	}

	// 获取存储空间信息并设置动态限制
	let maxFileSize = 30 * 1024 * 1024; // 默认30MB
	let storageInfo = null;

	try {
		// 使用浏览器的Storage API获取存储空间信息
		const estimate = await navigator.storage.estimate();
		storageInfo = estimate;

		// 计算可用存储空间的10%作为最大文件大小限制
		// 确保限制至少为3MB，最多为100MB
		if (estimate.quota && estimate.usage) {
			const availableSpace = estimate.quota - estimate.usage;
			const calculatedLimit = Math.floor(availableSpace * 0.1); // 使用10%的可用空间
			maxFileSize = Math.max(3 * 1024 * 1024, Math.min(calculatedLimit, 100 * 1024 * 1024));
		}
	} catch (error) {
	...
	if (file.size > maxFileSize) {
		...
		ShowAlert('错误', `图片文件过大，请选择${maxSizeMB}MB以下的图片！`, true, 3000);
		// 清空文件输入，允许用户重新选择
		event.target.value = '';
		return;
	}
	...
	reader.onload = async function(e) {
		...
		try {
			// 存储到IndexedDB
			await saveWallpaperToIndexedDB(base64, file.name);
			// 存储自定义壁纸标志到localStorage
			...
			// 更新提示信息
			if (customWallpaperInfo) {
				customWallpaperInfo.textContent = `已选择: ${file.name}`;
			}

			ShowAlert('成功', '壁纸设置成功！', true, 2000);
		} catch (error) {
			console.error('壁纸保存失败:', error);
			ShowAlert('错误', '壁纸保存失败，请重试！', true, 3000);
			// 清空文件输入，允许用户重新选择
			event.target.value = '';
		}
	};
	reader.onerror = function() {
		ShowAlert('错误', '读取文件失败，请重试！', true, 3000);
	};
	reader.readAsDataURL(file);
}

// 处理Base64壁纸
async function handleBase64Wallpaper() {
	if (!wallpaperBase64Input) return;

	const base64 = wallpaperBase64Input.value.trim();
	if (!base64) {
		ShowAlert('错误', '请先粘贴Base64编码！', true, 3000);
		return;
	}

	// 验证Base64格式
	if (!base64.startsWith('data:image/')) {
		ShowAlert('错误', 'Base64编码格式不正确！应以data:image/开头', true, 3000);
		return;
	}

	// 验证Base64长度，考虑localStorage存储限制（通常为5MB）
	// base64编码大小约为原始图片的1.33倍，所以限制为4MB base64编码（约3MB原始图片）
	if (base64.length > 4 * 1024 * 1024) { // 限制为4MB base64编码
		ShowAlert('错误', 'Base64编码过大，请使用3MB以下的图片！', true, 3000);
		return;
	}

	try {
		// 存储到IndexedDB
		await saveWallpaperToIndexedDB(base64, 'base64');
		// 存储自定义壁纸标志到localStorage
		localStorage.setItem('customWallpaper', 'true');
		...
		// 清空输入框
		wallpaperBase64Input.value = '';

		// 更新提示信息
		if (customWallpaperInfo) {
			customWallpaperInfo.textContent = '已应用Base64壁纸';
		}

		ShowAlert('成功', 'Base64壁纸设置成功！', true, 2000);
	} catch (error) {
		console.error('Base64壁纸设置失败:', error);
		ShowAlert('错误', 'Base64壁纸设置失败，请检查编码格式！', true, 3000);
	}
}
...
async function loadWeather() {
	...
	try {
		...
		if (isAutoLocation && navigator.geolocation) {
			...
			try {
				...
				if (!response.ok) {
					...
					throw new Error(`OpenWeatherMap API 错误: ${errorData.message || 'Unknown error'}`);
				}

				weatherData = await response.json();
			} catch (geoError) {
				console.error('定位失败，使用城市名获取天气:', geoError);
				// 自动定位失败，使用手动城市
				// 直接调用 OpenWeatherMap API（使用城市名）
				const response = await fetch(
		...
		// 验证天气数据格式
		if (!weatherData.name || !weatherData.main || !weatherData.weather) {
		...
		ShowAlert('错误', '啊偶～云朵把天气藏起来啦！请检查网络连接后重试。', true, 3000);
		weatherContent.innerHTML = `
                    <div class="weather-error">
                        <i class="fa-solid fa-exclamation-triangle"></i>
                        <p>加载天气失败: ${error.message}</p>
                        <p class="weather-error-hint">提示：请确保已在设置中填写有效的 OpenWeatherMap API Key，并且网络连接正常。</p>
                    </div>
                `;
	}
}
...
function initAiAssistant() {
	...
	function sendAiMessage() {
		...
		// 更新对话历史
		chatHistory.push(`用户: ${question}`);
		// 限制对话历史长度为最近的3轮对话（6条消息）
		...
		typingMessage.className = 'ai-message ai-message typing';
		...
		// 构建system参数
		const systemParam = chatHistory.join('\n');
		// 调用AI API
		const apiUrl = `https://yunzhiapi.cn/API/doubao.php?question=${encodeURIComponent(question)}&system=${encodeURIComponent(systemParam)}`;
		...
		fetch(apiUrl)
			...
			.then(data => {
				...
				aiResponse.className = 'ai-message ai-message';
